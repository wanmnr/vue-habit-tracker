    ...
    <a-row :gutter="[16, 16]" align="middle">
      <a-col :xs="24" :md="8">
        <div class="user-avatar-section">
          <a-avatar :size="96" :src="avatarUrl" alt="User avatar">
            {{ initials }}
          </a-avatar>
          <h2 class="user-name">{{ summary.fullName }}</h2>
          <a-tag :color="summary.isAdmin ? 'gold' : 'blue'">
            {{ summary.user?.role === 'admin' ? 'Administrator' : 'User' }}
          </a-tag>
        </div>
      </a-col>
      ...
      <a-col :xs="24" :md="16">
        <a-descriptions bordered>
          ...
          <a-descriptions-item label="Notifications" :span="3">
            <a-space>
              <a-tag v-for="channel in summary.notificationChannels" :key="channel" color="processing">
                {{ channel }}
              </a-tag>
            </a-space>
          </a-descriptions-item>
...
const avatarUrl = computed(() => {
  return props.summary.fullName
    ? `https://avatars.dicebear.com/api/initials/${props.summary.fullName}.svg`
    : '';
});
</script>
